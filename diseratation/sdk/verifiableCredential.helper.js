...
const generateVC = async (credentialSubject, vcParams) => {
  try {
    ...
    verifiableCredential[vcConstants.schema.context] = [
      vcConstants.firstContextEntry,
    ];
    ...
    verifiableCredential[vcConstants.jsonPropertyCredentialSubject] = [
      credentialSubject,
    ];
    ...
    verifiableCredential[vcConstants.jsonPropertyCredentialStatus] =
      vcConstants.credentialStatus;
...
exports.vcToHash = async (verifiableCredential) => {
  try {
    const credentialHash = await util.hashWithSHA256(
