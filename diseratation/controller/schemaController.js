...
const {createSchemaInTable,getSchemaByKey,getAllSchema} = require('../helper/schema.helper')
...
const getSchemaDetails = async (schemaName) =>{
    try {
      return await VerifiableCredential.find({'attributes.Item.schemaName': schemaName})
    } catch (error) {
