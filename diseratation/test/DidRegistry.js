...
describe("DidRegistry", () => {
  ...
  let otherAccount;
  const didDoc =
    '{"@context":"https://w3id.org/did/v1","id":"did:example:testnet:0x123"}';
  ...
  it("should update a DID", async () => {
    const updatedDoc =
      '{"@context":"https://w3id.org/did/v1","id":"did:example:testnet:0x123_updated"}';
  ...
  it("should store and retrieve VC details", async () => {
    ...
    expect(
      await didRegistry.getVcDetails(owner.address, didIdentifier),
    ).to.equal(vcHash);
  });
  ...
  it("should revert if DID is already finalized", async () => {
    ...
    await expect(didRegistry.finalizeDID(owner.address)).to.be.revertedWith(
      "DID is already finalized",
    );
  });
  ...
  it("should revert if a non-owner tries to store VC hash", async () => {
    await expect(
      didRegistry
        ...
        .storeVcHash(owner.address, didIdentifier, vcHash),
