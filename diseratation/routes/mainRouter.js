...
router.get("/api/issuer/createDid", async (req, res, next) => {
  try {
    ...
    const { address, publicKeyBase58, _privateKey, did } = await createDID(
      process.env.NETWORK,
      privateKey,
    );
    res.send(did);
...
router.post("/api/verfier/verifyVC", async (req, res, next) => {
  try {
    const did = req.body.did;
    const privateKey = req.query.privateKey
      ? req.query.privateKey
...
router.get("/api/issuer/schema/:schemaName", async (req, res, next) => {
  try {
    ...
    const jsonSchema = await schemaController.getSchemaDetails(
    ...
    res
      ...
      .json({
        success: true,
        data: jsonSchema,
        message: `${jsonSchema ? "Schema retrieved" : "Schema does not exist"}`,
        status: 200,
      });
  } catch (error) {
    next(error);
  ...
  async (req, res, next) => {
    try {
      ...
      if (jsonSchema.length == 0) {
        ...
        const [verifiableCredData, vcToHash] =
          await vcController.createVerifiableCred(req.body.credentialSubject, {
